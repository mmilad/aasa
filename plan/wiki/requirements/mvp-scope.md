...
8. **Local admin UI (Next.js)**: first-party **operator panel** in **Next.js** against the Worker REST API — [ADR-0005](../decisions/ADR-0005-local-admin-ui-nextjs.md). Loopback-first; same secret handling discipline as ADR-0003.

## Explicit non-goals (MVP v0.1)

- **No** **public Internet** control dashboard (anonymous or unauthenticated browser exposure to the world). **Local / same-host** Next.js operator UI is **in scope** per [ADR-0005](../decisions/ADR-0005-local-admin-ui-nextjs.md).
- **No** normative first-party **desktop** UI (WPF/WinUI/Avalonia) for the operator UI in MVP — web UI per ADR-0005.
...
5. Operator can open the **local Next.js admin** on loopback (per ADR-0005), authenticate via the configured API key pattern (no user-login system required for the POC), and **list servers** plus **enqueue at least one job** (install, start, or backup) with visible terminal/job outcome in the UI or linked detail view.
